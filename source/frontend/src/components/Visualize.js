    ...
    <>
        ...
        <Row>
          <Col xs={12} sm={12} md={6} lg={3} xl={3}>
            {bolgeAdlari.length !== 0 ? (
              <SelectSearch
                ...
                onChange={(e) => {
                  setSelectedBolge(e)
                  setSelectedYer(false)
                  setSelectedParametre(false)
                  setSelectedYil(false)
                  fetchYer(e)
                  setIsClicked(false)
                }}
              />
          ...
          <Col xs={12} sm={12} md={6} lg={3} xl={3}>
            <SelectSearch
              ...
              filterOptions={fuzzySearch}
              onChange={(e) => {
                e.length > 2 ? setShow(true) : setShow(false)
                setSelectedYil(e)
                setIsClicked(false)
              }}
            />
      ...
      <Row className={'button-container'}>
        <ButtonGroup aria-label="Basic example">
          ...
          <Button
            onClick={() => showInfo('Cizgi')}
            style={{ fontWeight: '500', fontSize: '15px' }}
            disabled={selectedYil[0] === 'all' || !selectedBolge || !selectedParametre || !selectedYer || !selectedYil || selectedYil?.length > 1}>
            Çizgi
          </Button>
      ...
      <Container fluid={queryInfo[0] === 'Tablo'} className="graph-container">
        {queryInfo.length !== 0 && isClicked ? <GraphContainer queries={queryInfo} locationType={locationType} /> : null}
      </Container>
    </>
  )
}
